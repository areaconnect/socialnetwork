...
$(document).ready(function(){
	...
	$('.like-form').on('submit',function(e){
		...
		return false;		
		 ...
		 /*comment remove */
	$('.comment-remove-form').on('submit',function(e){
		e.preventDefault();
		const comment_remove_url = $(this).attr('action');
		var comment_id 	 = $(this).find('input[name="comment_id"]')[0].value;
		...
		$.ajax({
				...
				success : function(response){
					if (response.status == 'ok'){
						$self.parents('.post-comment-wrap').hide();
					}
				}
		});
		return false;		
